...
import com.shashankmunda.musica.cast.CastServer.Companion.PARAM_ID
import com.shashankmunda.musica.cast.CastServer.Companion.PART_ALBUM_ART
import com.shashankmunda.musica.cast.CastServer.Companion.PART_SONG
import com.shashankmunda.musica.models.Song
import com.shashankmunda.musica.util.Utils.getIPAddress
...
class CastHelper {
    ...
    fun castSong(castSession: CastSession, song: Song) {
        try {
            ...
            remoteMediaClient.load(song.toMediaInfo(), MediaLoadOptions.Builder().apply {
                setPlayPosition(0)
                setAutoplay(true)
            }.build())
        } catch (e: Exception) {
